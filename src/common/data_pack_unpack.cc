...
const int8_t kE = 1;
uint8_t g1[32] = 
  {kA, kB, kC, kA + 4, kB + 4, kC + 4, kA + 8, kB + 8, kC + 8, kA + 12, kB + 12, kC + 12, 0x80, 0x80, 0x80, 0x80,
   kA, kB, kC, kA + 4, kB + 4, kC + 4, kA + 8, kB + 8, kC + 8, kA + 12, kB + 12, kC + 12, 0x80, 0x80, 0x80, 0x80};
uint8_t g2[32] = 
  {kD, kE, 0x80, kD + 4, kE + 4, 0x80, kD + 8, kE + 8, 0x80, kD + 12, kE + 12, 0x80, 0x80, 0x80, 0x80, 0x80,
   kD, kE, 0x80, kD + 4, kE + 4, 0x80, kD + 8, kE + 8, 0x80, kD + 12, kE + 12, 0x80, 0x80, 0x80, 0x80, 0x80};
...
const static __m256i mask = 
  _mm256_setr_epi8(0x00, 0xF0, 0xFF, 0x00, 0xF0, 0xFF, 0x00, 0xF0, 0xFF, 0x00, 0xF0, 0xFF, 0x00, 0x00, 0x00, 0x00,
...
void pack32_24_avx2(uint8_t *unpacked, uint8_t *packed, size_t to_pack) {
  ...
  for (size_t i = 0; i < to_pack / 32; i++) {
      __m256i aligned_unpacked = _mm256_load_si256(reinterpret_cast<const __m256i *>(&unpacked[32 * i]));
      ...
      _mm_storeu_si128(reinterpret_cast<__m128i *>(&packed[24 * i]), reinterpret_cast<__m128i>(*hi));
      _mm_storeu_si128(reinterpret_cast<__m128i *>(&packed[24 * i + 12]), reinterpret_cast<__m128i>(*lo));
  }
}
...
void pack32_24_naive(std::complex<int16_t>* unpacked, uint8_t *packed, size_t to_pack)
{
    for (size_t i = 0; i < to_pack / 4; i++) {
        //0xABCD -> 0xBC
        packed[3 * i + 0u] = static_cast<uint8_t>((static_cast<uint16_t>(unpacked[i].real()) >> 4u) & 0xFF);
        //0xABCD & 0xEFGH -> 0xGA
        packed[3 * i + 1u] = static_cast<uint8_t>((static_cast<uint16_t>(unpacked[i].imag()) & 0xF0)  |
                                          (static_cast<uint16_t>(unpacked[i].real()) >> 12u));
        //0xEFGH -> 0xEF
        packed[3 * i + 2u] = static_cast<uint8_t>((static_cast<uint16_t>(unpacked[i].imag()) >> 8u) & 0xFF);
    }
}